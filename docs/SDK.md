...
class PortPulseClient:
    ...
    def trend(self, u, days=7):
        r = requests.get(f"{self.base}/v1/ports/{u}/trend",
...
if __name__ == "__main__":
    ...
    st, csv, et = c.trend_csv("USLAX", 7); print("csv status:", st, "etag:", et)
class PortPulse {
  ...
  trend(u, { days=7 } = {}) { return this._f(`/v1/ports/${u}/trend`, { params: { days } }).then(r => r.json()); }
  trendCsv(u, { days=7, etag=null } = {}) {
    return this._f(`/v1/ports/${u}/trend`, {
      params: { days, format: "csv" },
      headers: etag ? { "If-None-Match": etag } : {}
    }).then(async r => ({ status: r.status, csv: await r.text(), etag: r.headers.get("etag") }));
  }
}
// Usage:
